...
const spaceKey = ({ bgPanel, toolPanel }) => {
  CONFIG.pause = !(CONFIG.pause === true) ;
  return {
    bgPanel,
    toolPanel
  };
...
const keyTetrisTable = (key, state) => (
  isValidKey(key) ? _.find(keyFnList, (item) => (_.isEqual(item.key, key))).fn({ bgPanel: state.bgPanel, toolPanel: state.toolPanel }) : { bgPanel: state.bgPanel, toolPanel: state.toolPanel }
);

const joinTetrisTable = (state) => (assignPanel({ bgPanel: state.bgPanel, toolPanel: state.toolPanel }));
