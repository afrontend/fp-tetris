...
const getEmptyRow = columns => createArray(columns, p.createItem(GLOBAL.color));
...
const isNotFullRow = fp.some(p.isBlankItem);
...
const createRandomToolPanel = bgPanel => {
  const toolPanel = pieceFactories[_.random(0, pieceFactories.length - 1)]();
...
const isValidKey = keyName =>
  _.some(keyFnList, item => _.isEqual(item.key, keyName));
...
const key = (keyName, state) =>
  isValidKey(keyName)
    ? _.find(keyFnList, item => _.isEqual(item.key, keyName)).fn({
