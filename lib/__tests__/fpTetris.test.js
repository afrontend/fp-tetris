...
// Always pick the O-piece (index 0): deterministic, fills a 4×4 board in 3 drops.
const alwaysO = () => 0;

describe("removeFullRows — board integrity", () => {
  it("board dimensions are preserved after multiple hard drops", () => {
    let state = game.init({ rows: 4, columns: 4, pieceSelector: alwaysO });
    for (let i = 0; i < 10; i++) {
      state = game.key("space", state);
    }
    const [bg] = game.toArray(state);
    expect(bg.length).toBe(4);
    expect(bg[0].length).toBe(4);
  });
  ...
  it("each row in bgPanel always has the correct number of columns", () => {
    ...
    const [bg] = game.toArray(state);
    bg.forEach((row) => expect(row.length).toBe(4));
  });

  it("hardDrop on a full board (blank toolPanel) returns state unchanged", () => {
    // Force game over: fill the board until toolPanel becomes blank
    let state = game.init({ rows: 4, columns: 4, pieceSelector: alwaysO });
    while (!game.isBlankToolPanel(state)) {
      state = game.key("space", state);
    }
    const [bgBefore, toolBefore] = game.toArray(state);
    state = game.key("space", state);
    const [bgAfter, toolAfter] = game.toArray(state);
    expect(bgAfter).toEqual(bgBefore);
    expect(toolAfter).toEqual(toolBefore);
  });
});
